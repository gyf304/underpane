...
import { LANGS, SHARED_STRINGS } from "@/lib/i18n";
import type { Lang, SharedStringKey } from "@/lib/i18n";
import { LangProvider, useLang } from "@/hooks/lang";
import "../index.css";
      ...
      <header className="mx-auto flex w-full max-w-5xl items-center justify-between px-6 py-5">
        <div className="flex items-center gap-6">
          ...
          <nav className="flex items-center gap-1">
            <Button
              asChild
              variant={
                activePath === "/" || activePath === "" ? "secondary" : "ghost"
              }
              size="sm"
            >
            ...
            <Button
              asChild
              variant={
                activePath.startsWith("/discover") ? "secondary" : "ghost"
              }
              size="sm"
            >
        ...
        <div className="flex items-center gap-1">
          <Select value={lang} onValueChange={(v) => setLang(v as Lang)}>
            <SelectTrigger
              size="sm"
              className="gap-2 border-none shadow-none focus:ring-0"
            >
      ...
      <footer className="border-t mt-auto">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between px-6 py-6 text-sm text-muted-foreground">
          <span className="text-xs">
            &copy; {new Date().getFullYear()} Underpane. All rights reserved.
          </span>
          <a
            href={REPO}
            target="_blank"
